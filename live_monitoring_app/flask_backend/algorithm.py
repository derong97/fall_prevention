...
def startAlgo(bed_number, patient_accompanied):
    ...
    while runAlgo:
        ...
        print("weight and vibr sensor: {}, {}, {}, {}, {}, {}".format(w_br, w_bl, w_fr, w_fl, v_b, v_t))
...
def stopAlgo(isAbort, isAccompanied):
    global runAlgo
    runAlgo = False
    ...
    if isAbort == 0: #session was properly terminated
        if isAccompanied:
            print("Sending only time started and stopped to SQL...") 
            try:
                print("isHFR: ", isHFR)
                new_log = sql_conn.execute("INSERT INTO  `post_monitoring_db`.`current_patient_logs` (bed_number,timestamp_start,timestamp_end,accompanied) \
                ...
                print("Sorry, data was not successfully sent to SQL.")
        else: 
            print("Sending full set of data is sent to SQL...")
            try:
                ...
                print("Sorry, data was not able to be sent to SQL.")
    else:
        print("data is not sent to SQL")
